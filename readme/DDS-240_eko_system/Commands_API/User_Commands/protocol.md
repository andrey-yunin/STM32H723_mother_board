...
### 4.3. Команда с неверным форматом (NACK)

Если команда пришла с неверной CRC, некорректной длиной или неизвестным кодом.
...
ПК                          Анализатор
 ...
 │──── Неверная команда ───────>│
 │                              │
 │<─── NACK (команда отклонена) │
 │                              │
```

### 4.4. Ошибка выполнения команды (ERR)

Если команда была принята, но в процессе выполнения произошла ошибка (например, механическая).
...
### 4.5. Попытка запуска команды, когда анализатор занят

Если хост отправляет команду, пока анализатор выполняет предыдущую.
