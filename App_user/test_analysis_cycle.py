...
def send_and_wait_ack(command_code: int, params: bytes = b''):
    global ser
...
def wait_for_done(command_code: int, expected_status: int = 0x0000):
...
def wait_for_data_and_done(command_code: int, expected_data_len: int, expected_done_status: int = 0x0000):
...
def main():
    ...
    try:
        ...
        # --- ЗАПУСК ТЕСТОВОГО СЦЕНАРИЯ "ЦИКЛ АНАЛИЗА" ---
